...
$(document).ready(function () {
    ...
    $('#myDeleteModal').on('hidden.bs.modal', function () {
        // 执行一些动作...
        $("#zhiChuDaLeiguanli").click();
    })


    $("#searchBtn").click(function () {
        var query_param_zhiChuDaLei_name = $("#query_param_zhiChuDaLei_name").val();
        $.ajax({
            url: "/queryZCDaLeiByNameWithLike",
            type: "GET",
            data: {name: query_param_zhiChuDaLei_name},
            ...
            success: function (data, textStatus) {
                if (data.success) {
                    ...
                    $.each(data.data, function (id, zhiChuDaLei) {
                        var $tr = $("<tr ></tr>");
                        var $td1 = $("<td >" + zhiChuDaLei.id + "</td>");
                        var $td2 = $("<td >" + zhiChuDaLei.name + "</td>");
                        var $td3 = $("<td><a class='deleteObjectLinkClass' href='#' name='" + zhiChuDaLei.name +
                            "' institution_code = '" + zhiChuDaLei.institution_code + "'  id = '" + zhiChuDaLei.id + "'>删除</a></td>");
